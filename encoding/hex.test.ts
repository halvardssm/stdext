...
000000a0  61 7a 2e 00 00 00 00 00                           |az......|`;
  ...
  await t.step("Uint8Array", () => {
    const result = dump(buffer8Compatible);
    assertEquals(result, buffer8Result);
  });

  await t.step("Uint16Array", () => {
    const result = dump(new Uint16Array(buffer16Compatible));
    assertEquals(result, buffer16Result);
  });

  await t.step("Uint32Array", () => {
    const result = dump(new Uint32Array(buffer32Compatible));
    assertEquals(result, buffer32Result);
  });

  await t.step("Uint8ClampedArray", () => {
    const result = dump(new Uint8ClampedArray(buffer8Compatible.buffer));
    assertEquals(result, buffer8Result);
  });

  await t.step("Int8Array", () => {
    const result = dump(new Int8Array(buffer8Compatible.buffer));
    assertEquals(result, buffer8Result);
  });

  await t.step("Int16Array", () => {
    const result = dump(new Int16Array(buffer16Compatible));
    assertEquals(result, buffer16Result);
  });

  await t.step("Int32Array", () => {
    const result = dump(new Int32Array(buffer32Compatible));
    assertEquals(result, buffer32Result);
  });

  await t.step("Float32Array", () => {
    const result = dump(new Float32Array(buffer32Compatible));
    assertEquals(result, buffer32Result);
  });

  await t.step("Float64Array", () => {
    const result = dump(new Float64Array(buffer64Compatible));
    assertEquals(result, buffer64Result);
  });

  await t.step("BigInt64Array", () => {
    const result = dump(new BigInt64Array(buffer64Compatible));
    assertEquals(result, buffer64Result);
  });

  await t.step("BigUint64Array", () => {
    const result = dump(new BigUint64Array(buffer64Compatible));
    assertEquals(result, buffer64Result);
  });

  await t.step("DataView", () => {
    const result = dump(new DataView(buffer8Compatible.buffer));
    assertEquals(result, buffer8Result);
  });

  await t.step("ArrayBuffer", () => {
    const result = dump(buffer8Compatible.buffer);
    assertEquals(result, buffer8Result);
  });
});
